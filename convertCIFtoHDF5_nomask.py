...
def __main__():
    if (len(sys.argv) < 4 or len(sys.argv) > 5):
        print("Wrong number of input arguments. Usage: python convertCIF2HDF5 input.cif output.hdf5 img_size [channels=All]")
    else:
        if (len(sys.argv) == 5):
            convertCIF2HDF5_nomask(inputFile = sys.argv[1], outputFile = sys.argv[2], img_size = int(sys.argv[3]), channelsString = sys.argv[4])
        else:
            convertCIF2HDF5_nomask(inputFile = sys.argv[1], outputFile = sys.argv[2], img_size = int(sys.argv[3]))
